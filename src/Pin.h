...
public:
    //Constructors
    Pin() {}
    Pin(int number, std::string name, std::string description,
         PinType type) : pinNumber_{number},pinName_{name},
         pinDescription_{description}, type_{type} {}
    Pin(Pin& cpy) : pinNumber_{cpy.pinNumber_}, pinName_{cpy.pinName_},
                    pinDescription_{cpy.pinDescription_}, type_{cpy.type_} {}
    
    Pin(Pin&& moveObj) noexcept : pinNumber_{std::move(moveObj.pinNumber_)},
